...
export default function HomePage() {
  ...
  const clearAllRecords = async () => {
    setIsClearing(true);
    try {
      const response = await fetch('https://attendance.shinoshike.studio/api/v1/api/v1/attendance/delete-all?confirm=true', {
        method: 'DELETE',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://attendance.shinoshike.studio/api/v1/api/v1/attendance/upload-attendance', {
        method: 'POST',
        body: formData,
      });
